    ...
    return `
      <div class="bg-white p-4 rounded shadow mb-2">
        ...
        <div class="mt-2 flex gap-2">
          ...
          <a href="https://wa.me/?text=Oi%20${encodeURIComponent(d.nome)},%20voc%C3%A1%20est%C3%A1%20devendo%20${encodeURIComponent(formatarMoeda(valorFinal))}.%20Favor%20acertar." target="_blank" class="bg-yellow-500 text-black px-3 py-1 rounded">Cobrar no Zap</a>
        </div>
      </div>
    `;
  }).join("");
...
function editarDivida(id) {
  ...
  document.getElementById("salvarDivida").onclick = () => {
    ...
    const atualizadas = dividas.map(d => d.id === id ? { ...d, nome, valor, juros, data, vencimento } : d);
