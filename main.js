...
<input placeholder="WhatsApp (ex: 11999998888)" id="zap" class="border p-2 w-full mb-2">
        <input placeholder="Data do empréstimo" id="data" type="date" type="date" class="border p-2 w-full mb-2">
        <input placeholder="Data de vencimento" id="vencimento" type="date" type="date" class="border p-2 w-full mb-2">
  ...
  document.getElementById("addDivida").onclick = () => {
    ...
    const whatsapp = document.getElementById("zap").value;
    if (!nome || !valor || !juros || !data || !vencimento || !whatsapp) return alert("Preencha tudo!");

    dividas.push({ id: Date.now(), nome, valor, juros, data, vencimento, whatsapp, pago: false });
    ...
    return `
      <div class="bg-white p-4 rounded shadow mb-2 ${d.pago ? 'opacity-50' : ''}">
        ...
        <div class="mt-2 flex gap-2">
          ...
          <a href="${d.whatsapp ? `https://wa.me/55${d.whatsapp}?text=Oi%20${d.nome},%20est%C3%A1%20devendo%20R$%20${valorFinal.toFixed(2)}.%20Favor%20acertar.` : "#"}" target="_blank" class="bg-yellow-500 text-black px-3 py-1 rounded">Cobrar no Zap</a>
        </div>
      </div>
    `;
  }).join("");
